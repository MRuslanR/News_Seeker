...
from config import (DB_PATH, get_logger, DB_CLEANUP_DAYS, MAX_CONCURRENT_WORKERS)
...
class NewsFetcher:
    ...
    # Записывает сбой и возвращает информацию для Excel
    ...
    @retry_on_exception(tries=2, delay_seconds=1, exceptions=(requests.exceptions.RequestException,))
    def _make_request(self, url: str):
        """Выполняет запрос с ретраями. Если упадет - исключение полетит выше."""
        return requests.get(url, headers={'User-Agent': 'MyNewsBot/1.0'}, timeout=(3, 5))
...
class OpenRouterClient:
    ...
    @retry_on_exception(tries=3, delay_seconds=2, backoff_factor=2) 
